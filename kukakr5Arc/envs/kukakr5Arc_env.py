...
#Adding code for initialising physics server
...
#		self.useOrientation = 1
	...
	def applyAction(self, actions):
		...
		flangePos[0] = currentflangePos[0]+actions[0]
		flangePos[1] = currentflangePos[1]+actions[1]
		flangePos[2] = currentflangePos[2]+actions[2]
		# Add more based on the change in action dim
		#self.flageOri = currentflangeOri+actions[3]

		targetjointPose = p.calculateInverseKinematics(self.robotId, self.flangeIndex, currentflangePos, currentflangeOri)

		#Given code handles the case where simulation is off
		p.setJointMotorControlArray(self.robotId, range(self.numJoints-2), p.POSITION_CONTROL, targetPositions=targetjointPose)

#All the modules work. Verified on 23-03-2020 14:52:00 IST
